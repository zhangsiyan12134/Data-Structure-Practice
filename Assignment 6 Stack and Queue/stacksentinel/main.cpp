...
stack::stack(void):sentinel(0)  //a sentinel created while stack initialized
{
}
void stack::push(int x)
{
	node *ptr;  //create a new node pointer
	ptr = new node(x);
	ptr->next = sentinel.next; //copy current top address in to new top next pointer
	sentinel.next = ptr;    //make the new node address as top in sentinel
}
int stack::pop(void)
{
	int temp = 0;   //create a temporary variable to store data in node
	temp = sentinel.next->data; //copy target node data into temp
	node *ptr = sentinel.next;  //create a new pointer to temporary store current top address
	sentinel.next = ptr->next;  //change the sentinel point to second node
	delete ptr; //delete old top
